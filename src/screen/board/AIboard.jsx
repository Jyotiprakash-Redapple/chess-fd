...
import $ from "jquery";
import "./ai.css";
import { Chess } from "../../arbitar/chess";
import {
	CButton,
	CCol,
	CRow,
...
function AIboard() {
	const [visibale, setVisible] = useState({
		seeting: false,
		location: false,
		computer: false,
	});
	...
	useEffect(() => {
		...
		board = Chessboard(document.getElementById("myBoard"), config);
	}, []);
	...
	function evaluateBoard(game, move, prevSum, color) {
		...
		if ("captured" in move) {
			...
			// Our piece was captured (bad for us)
				...
				prevSum -=
					weights[move.captured] + pstSelf[move.color][move.captured][to[0]][to[1]];
			}
		}
		...
		if (move.flags.includes("p")) {
			...
			if (move.color === color) {
				...
				prevSum +=
					weights[move.promotion] + pstSelf[move.color][move.promotion][to[0]][to[1]];
			}
